...
Ext.onReady(function () {
	...
	Ext.BLANK_IMAGE_URL = '/icon/s.gif';	
	Ext.QuickTips.init();
    ...
    var tree = {
        ...
        bbar: [
	        ...
	        new Ext.form.Label({
	        	...
	        	iconCls: Ext.ux.iconCls.get('/icon/arrow_down.png'),
	        	text: 'Next',
	        	handler: function () {
	        		jsonviewer.searchNext();
	        	}
	        }, {
	        	iconCls: Ext.ux.iconCls.get('/icon/arrow_up.png'),
	        	text: 'Previous',
	        	handler: function () {
	...
	var jsonviewer = function () {
		...
		return {
			...
			treebuild: function () {
				...
				root.setIcon(Ext.isArray(json) ? '/icon/array.gif' : '/icon/object.gif');
				this.gridbuild(root);
			},
			...
			json2leaf: function (json) {
				...
				for (var i in json) {
					if (json.hasOwnProperty(i)) {
						if (json[i] === null) {
							ret.push({text: i + ' : null', leaf: true, icon: '/icon/red.gif'});
						} else if (typeof json[i] === 'string') {
							ret.push({text: i + ' : "' + json[i] + '"', leaf: true, icon: '/icon/blue.gif'});
						} else if (typeof json[i] === 'number') {
							ret.push({text: i + ' : ' + json[i], leaf: true, icon: '/icon/green.gif'});
						} else if (typeof json[i] === 'boolean') {
							ret.push({text: i + ' : ' + (json[i] ? 'true' : 'false'), leaf: true, icon: '/icon/yellow.gif'});
						} else if (typeof json[i] === 'object') {
							ret.push({text: i, children: this.json2leaf(json[i]), icon: Ext.isArray(json[i]) ? '/icon/array.gif' : '/icon/object.gif'});
						} else if (typeof json[i] === 'function') {
							ret.push({text: i + ' : function', leaf: true, icon: '/icon/red.gif'});
						}
					}
				}
				return ret;
